# Interface module to handle all rendering and updating of the UI
# and any user inputs
...
# Verify user's answers after each round of the quiz
# If the user has answered 5 questions, update scores and return to main menu
def check(vars, sentencePairs, canvas, base, ops, difficulty):
    ...
    for i in range(0, 4):
        ...
        if (vars[i].get() == 1) and not sentencePairs[i][1]:
            correct = False
        # Marked as false but is true
        if (vars[i].get() == 0) and sentencePairs[i][1]:
            correct = False
    # Display correct/incorrect message
    canvas.create_rectangle(WIDTH/2-200, HEIGHT/3-100,
...
# Render the generated image and sentences
def render(base, canvas, vars, ops, difficulty):
    ...
    for i in range(0, 4):
        ...
        vars[i].set(0)
    ops[4].config(command=lambda: check(vars, sentences, canvas,
                                        base, ops, difficulty))
    base.update()


# Position elements required for quiz
def setup_quiz(base, difficulty):
    ...
    op1.place(x=PADDING, y=(HEIGHT*0.6)+PADDING,
              width=(WIDTH*0.5)-2*PADDING, height=(HEIGHT*0.2)-2*PADDING)
    op2 = tk.Checkbutton(base, variable=var2)
    op2.place(x=(WIDTH*0.5)+PADDING, y=(HEIGHT*0.6)+PADDING,
              width=(WIDTH*0.5)-2*PADDING, height=(HEIGHT*0.2)-2*PADDING)
    op3 = tk.Checkbutton(base, variable=var3)
    op3.place(x=PADDING, y=(HEIGHT*0.8)+PADDING,
              width=(WIDTH*0.5)-2*PADDING, height=(HEIGHT*0.2)-2*PADDING)
    op4 = tk.Checkbutton(base, variable=var4)
    op4.place(x=(WIDTH*0.5)+PADDING, y=(HEIGHT*0.8)+PADDING,
              width=(WIDTH*0.5)-2*PADDING, height=(HEIGHT*0.2)-2*PADDING)
...
# Display difficulty options after Play is clicked
def display_difficulties(base, start):
    start.destroy()
    easy = tk.Button(base, text="Easy", font=("Arial", 20),
                     command=lambda: setup_quiz(base, 0))
    easy.place(x=(WIDTH*0.5)-150, y=(HEIGHT*0.5)-30, width=300, height=60)
    medium = tk.Button(base, text="Medium", font=("Arial", 20),
                       command=lambda: setup_quiz(base, 1))
    medium.place(x=(WIDTH*0.5)-150, y=(HEIGHT*0.5)+60, width=300, height=60)
    hard = tk.Button(base, text="Hard", font=("Arial", 20),
                     command=lambda: setup_quiz(base, 2))
    hard.place(x=(WIDTH*0.5)-150, y=(HEIGHT*0.5)+150, width=300, height=60)


# Retrieve names of user's current and next rank
def get_rank_names():
    f = open("usr.bin", "rb")
    user_list = list(f.read())
    f.close()
...
def user_profile(base):
    ...
    home = tk.Button(base, text="Home", font=("Arial", 20),
                     command=lambda: menu(base))
    ...
    # Current and next rank for below bar
    ...
    # Success rates in various difficulties
    ...
    easy_rate = tk.Label(base, text="Easy: " + str(easy) + "%",
                         font=("Arial", 20))
    easy_rate.place(x=0, y=(HEIGHT/2)+200, width=WIDTH, height=40)
    medium_rate = tk.Label(base, text="Medium: " + str(medium) + "%",
                           font=("Arial", 20))
    medium_rate.place(x=0, y=(HEIGHT/2)+240, width=WIDTH, height=40)
    hard_rate = tk.Label(base, text="Hard: " + str(hard) + "%",
                         font=("Arial", 20))
...
def menu(base):
    ...
    start = tk.Button(base, text="Start", font=("Arial", 20),
                      command=lambda: display_difficulties(base, start))
    start.place(x=(WIDTH*0.5)-150, y=(HEIGHT*0.5)-30, width=300, height=60)

    profile = tk.Button(base, text="Profile", font=("Arial", 20),
                        command=lambda: user_profile(base))
